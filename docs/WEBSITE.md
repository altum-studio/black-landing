...
| Mobile (auditoría Playwright iPhone SE/14 y Pixel 7): enlaces de texto con 17–28 px de alto, etiquetas 12 px, aviso 11 px, CTAs del hero bajo el pliegue | 2 | Áreas de toque ≥44 px en enlaces y teléfonos, etiquetas 13 px en mobile, aviso 12 px, CTAs del hero en dos columnas, etiqueta de sección corta | done |
...
| SEO/AEO: SPA sin contenido en el HTML crudo (invisible para rastreadores de IA y lento de indexar), título sin keyword, H1 sin "locales comerciales", sin datos estructurados, sin robots/sitemap | 3 | Pre-render en build + hidratación; título "Locales comerciales en pozo en Escobar \| BLACK…"; kicker dentro del H1; JSON-LD ShoppingCenter + WebSite; robots.txt y sitemap.xml; lang es-AR; geo tags | done |
| Tracking: sólo Pixel (pierde 20–30 % de leads por bloqueadores/iOS), sin deduplicación ni fbclid | 3 | event_id compartido Pixel/CAPI, captura de fbclid→fbc y _fbp, función `/api/lead` a Conversions API con PII hasheada. Falta cargar META_PIXEL_ID / META_CAPI_TOKEN / VITE_META_PIXEL_ID / VITE_GA4_ID en Vercel | pendiente cliente |

## Lead Capture
Form de 5 campos que replica el "Black form" de Meta. Al enviar abre WhatsApp con el mensaje prearmado (unidad, objetivo, capital), POST a `/api/lead` (Meta CAPI con `event_id` compartido con el Pixel, fbc/fbp, teléfono hasheado), eventos `generate_lead` (GA4) y `Lead` (Meta). Sin scorecard: el capital y el objetivo ya califican el lead para Coldwell.
| Sección FAQ (6 preguntas + FAQPage) agregada en la pasada SEO | 0 | Retirada a pedido del cliente el 14/09/2026; el JSON-LD FAQPage se quitó con ella | done |
